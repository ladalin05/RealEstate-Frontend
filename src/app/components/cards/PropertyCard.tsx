    ...
    return (
        <div className="h-full relative bg-white shadow-lg rounded-md overflow-hidden">
            <div className="relative w-full h-68 group cursor-pointer overflow-hidden" onClick={() => toPropDetail(property.id)}>
                ...
                <p className="absolute bottom-2 left-4 z-10 text-xl font-medium text-white mb-2">{property.price}</p>
                <div className="absolute bottom-4 right-4 z-10 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <p className="w-7 h-7 bg-black/65 flex items-center justify-center rounded-md"> {property.favorite ? <HeartFill size={16} className="text-red-500" /> : <Heart size={16} className="text-white" />} </p>
            ...
            <div className="p-6">
                <h3 className="text-md font-bold text-gray-800 mb-2 hover:text-blue-500 cursor-pointer" onClick={() => toPropDetail(property.id)}>
                    {property.name}
                </h3>
                <p className="text-sm font-medium text-gray-500 mb-2 flex items-center"><GeoAlt className="me-2"/> {property.address}</p>
