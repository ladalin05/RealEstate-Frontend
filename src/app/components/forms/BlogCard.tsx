...
export const BlogCard = ({ blog}: { blog: any }) => {
    ...
    return (
        <div className="relative w-100 h-auto shadow-sm group overflow-hidden p-2 rounded-sm">
            ...
            <div className="h-58 py-5 px-2">
                <div className="flex items-center justify-between">
                    <p className="flex items-center text-sm"><CalendarEventFill size={12} className="me-2"/>{blog.date}</p>
                    <p className="flex items-center text-sm"><Tag size={13} className="me-2"/>{blog.category}</p>
                </div>
                <h2 className="text-md font-medium mt-2">{blog.title}</h2>
                <p className="text-sm font-medium mt-2 mb-2">{blog.description}</p>
                <button onClick={() => toDetail(blog.id)} className="text-sm text-sky-400">Continue reading</button>
            </div>
            <hr className="w-full border-gray-400 mt-2" />
            <div className="px-2 py-3 flex items-center justify-between">
                <div className="flex items-center">
                    <User size={16} className="font-thin"/>
                    <span className="text-gray-500 ms-2 text-sm">by {blog.author}</span>
                </div>
            </div>
        </div>
    );
