...
export const HeroSection = ({categories}: {categories: any[]}) => {
  ...
  return (
    <div className="flex items-center h-130 lg:h-[120vh]" 
      ...
      <div className="container">
        <div className="flex justify-center">
          <div className="text-center">
            ...
            <h1 className="text-5xl lg:text-7xl text-white font-bold my-12">{t('hero.title1')}<span className="text-sky-400">{t('hero.title2')}</span></h1>
            <p className="text-lg text-gray-500 my-12">{t('hero.description')}</p>
            
            {/* Modern Search Bar */}
            <div className="max-w-[800px] bg-white/15 backdrop-blur-[10px] border border-white/20 p-3 rounded-xl shadow-lg mx-auto">
              <div className="flex gap-5">
                <div className="w-[50%]">
                  <input type="text" value={filters.search_address} onChange={(e) => setFilters({...filters, search_address: e.target.value})} className="w-full h-11 py-1.5 pr-3 pl-6 bg-white text-base text-black rounded-lg placeholder:text-gray-500 focus:outline-none sm:text-sm/6" placeholder={t('hero.search_location')} />
                </div>
                <div className="w-[38%] h-11">
                    <CustomSelect key={'category_id'} label={t('hero.property_type')} className={"w-full bg-white rounded-sm"} options={PROPERTY_TYPES} value={filters.category_id} onChange={(value) => setFilters({...filters, category_id: value})} />
                </div>
